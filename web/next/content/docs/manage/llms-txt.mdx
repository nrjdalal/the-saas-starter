...
- **`/llms.txt`**: an index. Lists every docs page (each linked as its `.md` alias) plus a link to the blog. `/llms.txt/blog` serves the blog's own index as a route; `/llms.txt` itself is the docs listing.
...
The docs and blog are [features](/docs/manage/features): turn one off and its pages, section index, and per-page aliases drop from both files, so the files never point at a docs or blog page that 404s.
...
The per-page `.md`/`.txt` rewrites (in `web/next/next.config.ts`) cover `/docs/*` and `/blog/*` only. The top-level `/docs.md`, `/docs.txt`, `/blog.md`, and `/blog.txt` forms are not routes and 404. Use `/llms.txt` for the docs listing and `/llms.txt/blog` for the blog index.
