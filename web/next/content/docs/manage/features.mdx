...
title: Features
description: Toggle optional surfaces (allowlist, docs, blog, API reference, internal docs, waitlist) from one config, chosen at init and flippable anytime.
---
...
The fork defaults are **docs, blog, internal docs, and the API reference on; the waitlist and the allowlist off** (so a fresh fork lands on a plain home). Skipping the picker keeps those defaults. `--yes` takes them without prompting.
