...
Releases ship by promoting `canary` to `main`. The release number is decided before the merge by `.github/workflows/auto-canary-into-main.yml`; everything after it (changelog, tag, GitHub release) is automated by `.github/workflows/auto-release.yml`, and the CLI is published from the tag by `.github/workflows/cli-release.yml`; cutting a release is one deliberate merge, because `main` deploys to production.
