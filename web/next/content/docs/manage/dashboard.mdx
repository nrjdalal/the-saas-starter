...
The chrome is not inlined in the layout. It lives in `SidebarShell` (`web/next/src/components/shell/sidebar-shell.tsx`), a shadcn/ui `Sidebar` wrapped in `SidebarAdaptive`, an icon rail in the app and offcanvas inside the docs area, shared by both the dashboard and console layouts. The layout fills three slots:
...
The console layout is the same shell with two props flipped: a `badge` ("Console") and `homeHref="/console"`; both default to dashboard values. A `SidebarRail` gives the sidebar edge a click target that toggles it, and open/closed state persists in a `sidebar_state` cookie.
