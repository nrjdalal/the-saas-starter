...
Five tables in `packages/db/src/schema/auth.ts` hold the tenancy, cascading from their organization or team on delete (`invitation.teamId` carries no foreign key):
